...
def show_graph_basic_info(cc_topo):
    print("=========Basic Info========")
    print("node_number %d" %cc_topo.number_of_nodes())
    print("edge_number %d" %cc_topo.number_of_edges())
...
def dist_from_original_graph(cc_topo, u, v):
    return nx.shortest_path_length(cc_topo, u, v)
...
def dist_from_aggregated_graph(spt_topo, node_mapping_set, u, v):
    ...
    return nx.shortest_path_length(spt_topo, u, v)
...
def gen_shortest_path_tree_topo(root, cc_topo):
    ...
    nodes = []
    print(root)
    dist_u_map = nx.shortest_path_length(cc_topo, source=root)
    #print(dist_u_map)
        ...
        if node_u != root and node_u not in nodes:
            ...
            for node_pu in cc_topo.nodes():
                if cc_topo.has_edge(node_pu, node_u):
                    ...
                    edge_dist_pu_u = nx.shortest_path_length(cc_topo, node_pu, node_u)
                    dist_u = dist_u_map.get(node_u)
                    if dist_pu + edge_dist_pu_u == dist_u:
                        pu_map.setdefault(node_pu, nx.shortest_path_length(cc_topo, node_pu, root))
            #print(pu_map)
            if len(pu_map) > 0:
    ...
    while not check_latency(aggregated_spt_topo, latency):
        ...
        for (u, v) in aggregated_spt_topo.edges():
            ...
            if aggregated_spt_topo.edge[u][v]['latency'] < latency:
                dist_u_root = nx.shortest_path_length(aggregated_spt_topo, u, root)
                dist_v_root = nx.shortest_path_length(aggregated_spt_topo, v, root)
    ...
    for u in add_edge_back_aggregated_topo.nodes():
        for v in add_edge_back_aggregated_topo.nodes():
            if not add_edge_back_aggregated_topo.has_edge(u, v) and not add_edge_back_aggregated_topo.has_edge(v, u) and u != v:
                ...
                for ue in u_set:
                    for ve in v_set:
                        cost_list.append(nx.shortest_path_length(old_topo, ue, ve))
                new_edge_weight = statistics.median(cost_list)
                #new_edge_weight = statistics.mean(cost_list)
                if new_edge_weight > abs(nx.shortest_path_length(aggregated_spt_topo, u, root) - nx.shortest_path_length(aggregated_spt_topo, v, root)):
                    print((u, v))
...
show_graph_basic_info(spt_topo)


start = time.clock()
aggregated_spt_topo = aggregate_nodes_by_latency(spt_topo, root, 11)
end = time.clock()
print("=================aggregate_gen_shortest_path_tree_time")
print(end - start)
...
show_graph_basic_info(aggregated_spt_topo[0])
